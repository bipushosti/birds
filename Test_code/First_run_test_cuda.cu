...
//This is a copy of First_run_test.c but edits made to make it a CUDA code. 
...
//--------------------------------------------------------------------------------------------------------------------------
__device__ void get_movementData(FILE* outTxt,float* udata,float* vdata,float* dirData,float* precipData,float* pressureData,float* u10data,float* v10data,long int l,float pos_row,float pos_col);
__device__ float getProfitValue(float u,float v,float dirVal,float dir_u,float dir_v);
__device__ float bilinear_interpolation(float x,float y,float* data_array,long l,int dataSize);
...
//----------------------------------------------------------------------------------------------------//
	...
	while(l < TIMESTEPS){
		get_movementData(posdataTxt,udata,vdata,dirData,precipData,pressureData,u10data,v10data,l,pos_row,pos_col);
	}
