...
/*
	...
	//and <90 but negative when angle is >-90 and <0. So no need to check
	//if the value is negative
	else if((v>0)&&(u>0)){	//Quadrant 1
		u = abs(u);
		v = abs(v);
		angle = atanf(u/v) * (180/PI);
		diffAngle = abs(angle - dirVal);
	}
	else if((v<0)&&(u>0)){	//Quadrant 2
		u = abs(u);
		v = abs(v);
		angle = atanf(v/u) * (180/PI) + 90;
		diffAngle = abs(angle - dirVal);
	}
	else if((v<0)&&(u<0)){	//Quadrant 3
		u = abs(u);
		v = abs(v);
		angle = atanf(u/v) * (180/PI) + 180;
		diffAngle = abs(angle - dirVal);
	}
	else if((v>0)&&(u<0)){	//Quadrant 4
		u = abs(u);
		v = abs(v);
		angle = atanf(v/u) * (180/PI) + 270;
		diffAngle = abs(angle - dirVal);
	}
